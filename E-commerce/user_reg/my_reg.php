...
if(isset($_POST['submit'])){
	...
	$check=mysqli_num_rows(mysqli_query($con,"select * from registration where name='$name'"));
	if($check>0){
		$msg="Username is already present";
	}else{
		if($password===$cPassword){
		$qy = "INSERT INTO registration(name,password,cPassword,token) VALUES('$name','$pass','$cpass','$token')";
    ...
    <form method="POST">
		...
		<p class="hint-text">Create your account. It's free and only takes a minute.</p>
         <div class="form-group">
        	<input type="text" class="form-control" name="name" id="name" placeholder="Enter Username here" required>
        </div>
		<div class="form-group">
